...
function setup_influxd () {
        echo "This is My PORT ${INFLUXD_INIT_PORT}"
    ...
    echo "This is My test example ${setup_args[@]}"
    if [ -n "${DOCKER_INFLUXDB_INIT_RETENTION}" ]; then
        echo "This is My test for retention in"

        setup_args=("${setup_args[@]}" --retention "${DOCKER_INFLUXDB_INIT_RETENTION}")
                echo "This is My test for retention out"

    fi
    if [ -n "${DOCKER_INFLUXDB_INIT_ADMIN_TOKEN}" ]; then
            echo "This is My test for retention token in"

        setup_args=("${setup_args[@]}" --token "${DOCKER_INFLUXDB_INIT_ADMIN_TOKEN}")
                    echo "This is My test for retention token out"

    fi
            echo "This is My test for retention token before setup"

    influx setup "${setup_args[@]}"
                echo "This is My test for retention token after setup"
